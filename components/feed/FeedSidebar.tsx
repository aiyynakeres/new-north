    ...
    <aside className="w-full md:w-[300px] md:shrink-0 space-y-6 md:sticky md:top-20 md:self-start">
      ...
      <section className="bg-white rounded-xl border border-north-200 p-5 shadow-sm">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link to="/authors" className="text-xs text-north-500 hover:text-north-800 flex items-center gap-0.5">
        ...
        <ul className="space-y-3">
          {topAuthors.map(({ user, articleCount }) => (
            <li key={user.id}>
              <Link to={`/profile/${user.id}`} className="flex items-center justify-between gap-2 group">
      ...
      <section className="bg-white rounded-xl border border-north-200 p-5 shadow-sm">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link to="/communities" className="text-xs text-north-500 hover:text-north-800 flex items-center gap-0.5">
            Все <ChevronRight size={14} />
          </Link>
        </div>
        <ul className="space-y-3">
          {topCommunities.map((c) => (
            <li key={c.id}>
              <Link to={`/community/${c.id}`} className="flex items-center justify-between gap-2 group">
                <span className="text-sm font-medium text-north-800 truncate group-hover:text-north-600">{c.name}</span>
                <span className="text-xs text-north-400 shrink-0 tabular-nums">{c.memberIds.length}</span>
              </Link>
            </li>
          ))}
        </ul>
