...
const CommunityView: React.FC<Props> = ({ currentUser }) => {
	...
	const adminNicknames = useMemo(() => {
		...
		const ordered = [
			community.creatorId,
			...community.adminIds.filter((aid) => aid !== community.creatorId),
		];
	...
	const handleLeave = async () => {
		...
		try {
			...
			if (!r.ok && r.error === "creator_cannot_leave") {
				setMsg(
					"Создатель не может покинуть сообщество (передайте права или удалите группу в будущей версии).",
				);
			}
		} catch {
			setMsg(
			...
			<div className="max-w-3xl mx-auto px-4 -mt-12 relative">
				<div className="bg-white rounded-xl border border-north-200 p-6 shadow-sm">
					...
					<div className="mt-5">
						...
						>
							...
							<span className="text-lg font-bold tabular-nums leading-none text-north-900">
				...
				<section className="mt-10">
					<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-4">
						...
						{!currentUser && (
							<Button
								type="button"
								onClick={() =>
									navigate("/login", { state: { from: `/write?community=${community.id}` } })
								}
							>
					...
					<ul className="space-y-3">
						{articles.map((a) => (
							<li key={a.id}>
								...
								>
									...
									<span className="text-xs text-north-400 ml-2">
						...
						{articles.length === 0 && (
							<p className="text-north-400 text-sm italic">Пока нет постов в этом сообществе.</p>
						)}
					</ul>
				</section>
				...
				{isModerator && (
					<section className="mt-10 bg-white rounded-xl border border-north-200 p-6">
						<h2 className="font-serif text-lg font-bold text-north-900 mb-2">
							Участники и модерация
						</h2>
						<p className="text-sm text-north-500 mb-4">
							Создатель может назначать админов. Админы и создатель могут редактировать посты в
							сообществе и блокировать участников (кроме создателя).
						</p>
						<ul className="space-y-3">
							{community.memberIds.map((uid) => {
								...
								return (
									<li
										key={uid}
										className="flex flex-wrap items-center justify-between gap-2 text-sm border-b border-north-100 pb-2"
									>
										<div>
											<Link
												to={`/profile/${uid}`}
												className="font-medium text-north-800 hover:underline"
											>
												@{u.telegramHandle}
											</Link>
										...
										<div className="flex gap-2">
											{isCreator && uid !== community.creatorId && !admin && (
												<button
													type="button"
													className="text-xs text-north-600 hover:underline"
