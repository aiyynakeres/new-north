...
const Editor: React.FC<Props> = ({ currentUser }) => {
	...
	const handleSave = async () => {
		...
		const resolvedAudience: ArticleAudience = !resolvedCommunityId ? "public" : audience;
	...
	return (
		<div className="max-w-4xl mx-auto px-4 py-8 pb-32">
			<div className="mb-6 flex justify-between items-center sticky top-[64px] z-30 bg-north-50/90 backdrop-blur py-4 border-b border-north-200/50">
				<h1 className="text-2xl font-serif font-bold text-north-900">
			...
			<div className="space-y-6">
				...
				<div className="mb-8">
					<label
						htmlFor="editor-publication"
						className="block text-sm font-medium text-north-600 mb-2"
					>
						Публикация
					</label>
					{canPickCommunity ? (
						<select
							id="editor-publication"
						...
						<p className="text-sm text-north-600">
							Сообщество:{" "}
							<Link
								to={`/community/${existingArticle.communityId}`}
								className="text-north-800 underline"
							>
								{myCommunities.find((c) => c.id === existingArticle.communityId)?.name ||
									existingArticle.communityId}
							</Link>{" "}
				...
				{canPickCommunity && (
					<div className="mb-8 rounded-xl border border-north-200 bg-white p-4">
						<span className="block text-sm font-medium text-north-700 mb-3">Видимость</span>
						<div className="space-y-3">
							...
							>
								...
								<span>
									...
									<span className="block text-xs text-north-500 mt-0.5">
										Только участники выбранного сообщества; в общей ленте не показывается. Сначала
										выберите сообщество выше.
									</span>
								</span>
							</label>
						</div>
						...
						<div key={block.id} className="group relative pl-8 md:pl-12 transition-all">
							<div className="absolute left-0 top-1 opacity-0 group-hover:opacity-100 flex flex-col gap-1 transition-opacity">
								<button
									...
									className="p-1 text-north-400 hover:text-north-800"
								>
									<MoveUp size={14} />
								</button>
								<button
				...
				<div className="flex justify-center mt-8">
					<div className="flex gap-2 bg-white p-2 rounded-full shadow-lg border border-north-100">
						<button
							...
							className="p-2 hover:bg-north-50 rounded-full text-north-600 tooltip"
							title="Text"
						>
							<Type size={20} />
						</button>
						<button
							type="button"
							onClick={() => addBlock("h1")}
							className="p-2 hover:bg-north-50 rounded-full text-north-600"
							title="Heading 1"
						>
							<Heading size={20} />
						</button>
			...
			<div className="mt-12 pt-6 border-t border-north-200">
				<div className="flex items-center justify-between mb-2">
					<label htmlFor="editor-tags" className="text-sm font-medium text-north-600">
						Tags
					</label>
				</div>
