...
const ArticleReactionsControl: React.FC<Props> = ({
	article,
	users,
	currentUser,
	onUpdated,
				...
				if (richHover) {
					return (
						<span
							...
							title={names ? `${emoji}: ${names}` : undefined}
						>
				...
				<div className="flex flex-wrap items-center gap-1 border-l border-north-200 pl-2">
					{QUICK_EMOJIS.map((e) => (
						<button
							...
							type="button"
							title={
								userHasReaction(article.reactions, currentUser.id, e)
									? "Убрать реакцию"
									: `Отреагировать ${e}`
							}
							...
							className={`rounded-lg px-1.5 py-1 text-xl leading-none transition-colors hover:bg-north-100 ${
								userHasReaction(article.reactions, currentUser.id, e)
									? "ring-2 ring-north-400 bg-north-100"
									: ""
							}`}
						>
							{e}
									...
									{MORE_EMOJIS.map((e) => (
										<button
											...
											className={`rounded-lg px-2 py-1 text-xl leading-none hover:bg-north-50 ${
												userHasReaction(article.reactions, currentUser.id, e)
													? "bg-north-100 ring-1 ring-north-300"
													: ""
											}`}
										>
											{e}
